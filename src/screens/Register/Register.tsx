...
import {setUserTypes} from '../../redux/reducer/authSlice';
const userTypeData = [
  {
    id: 1,
    user_type: 'Buyer',
...
const Register = () => {
  ...
  const onSignUPPress = () => {
    ...
    setIsLoading(true);
    dispatch(
      signUp(email, password, Cpassword, name, userType, success => {
        setIsLoading(false);
        if (success) {
          ...
          <View>
            <View style={styles.useTypeSelection}>
              <TouchableOpacity
                ...
                <SemiBoldText style={{color: colors.white}}>
                  {userType}
                </SemiBoldText>
                <Image
                  source={images.Arrow}
                  style={{
                    ...
                    transform: [{rotate: isShowPopUp ? '90deg' : '-90deg'}],
                  }}
                />
              </TouchableOpacity>
            </View>
                ...
                {userTypeData?.map((item: any, index: number) => (
                  <View key={index}>
                    <TouchableOpacity
                      ...
                      onPress={() => onUserTypePress(item?.user_type)}>
